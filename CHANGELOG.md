...
- **Integration guide (`bareagent.context.md`) now documents `recurse()` — it shipped in 0.20.0 with ZERO mentions (adopter-reported gap).** The flagship 0.20.0 primitive was fully present in `src/` + the README spotlight + the PRD, but the AI-assistant-facing **integration guide** — which ships in the npm tarball (`package.json` `files`) and is many adopters' ground truth — never mentioned it, so the guide could not answer "how do I wire recurse?". Added a full **"Wiring with recurse"** section grounded in the shipped JSDoc (the cost-is-open-by-design gate warning first; `recurse(task, ctx, opts)` → `{result, verdict, receipts}` | `{incomplete, best, missingSlices, receipts}`; the governed-`Gate` wiring; Family-A/B/partition control; the `scan`/`search`/`exact`/`tools` retrieval table; synthesis + `contract`/`evaluate`; the exported `buildScanTool`/`buildSearchTool`/`buildExactTool`/`litectxCorpus` helpers) plus two "Which components do I need?" rows. Every referenced symbol verified against `index.js`. Docs only. **Note:** the published 0.20.0 tarball still lacks this; it lands in the next release (a doc-only 0.20.1 patch-publish would close it on npm immediately if wanted).
