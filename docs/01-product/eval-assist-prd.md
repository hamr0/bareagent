...
**Status:** Draft. Companion spec to the main `prd.md` (which stays THE core one-shot-runner PRD). This doc collects a small, opt-in suite of *assistive* components discussed 2026-06-22 — capabilities a consumer composes onto the Loop, never machinery the Loop imposes. Written one feature at a time, in order; each section is locked before the next is drafted.
**Owner:** hamr0
...
> **Implementation status (2026-06-23, branch `feat/eval-assist-skill-mechanism`).** **Feature 4 — DONE** (`isCritical` export + freeze; `src/complexity.js`). **Feature 3 — core DONE** (the meter): provider cache-tier normalization, 4-tier `estimateCost` + corrected rate table, native `GeminiProvider`, `result.metrics` + the priced/unpriced cost contract (closes #3); live-validated (`poc/f3-*.mjs`). **Feature 1 — DONE** (the Evaluator): `predicate`/`rubric`/`agentic` criteria, tri-state `Verdict`, isolated adversarial grader + bounded `refine`; live-validated (`poc/f1-*.mjs`). **Feature 2 — DONE** (skill mechanism + stash): Part A `SkillRegistry` + tools-as-thunk (`src/skills.js`); Part B `createStashSkill` (`src/stash.js`) — both strategies (`summarize` default / `stash` lossless), checkpoint-bracketed on-demand folds, auto-compaction (token pressure), litectx `episode` stance. **Live-validated on the real Anthropic wire** for all three fold shapes (`poc/f2-stash-live-anthropic.mjs`) — which caught a real `tool_use.id` charset bug the static checks missed. **Update (2026-06-24):** the §3.6 observability block is now BUILT — `context.tokensTrimmed` (approx ~4 chars/token over the evicted span) + `metrics.memory.{stashed,episodes,recalls}` via the loop-lent `ctx.recordMemoryOp` hook (channel A, bounded per run; `recalls` opt-in through `Memory.search(query,{ctx})`). The meter→gate **pricing contract** also landed on the adapter (`onLlmResult` forwards `costUsd:null`/`pricing:'unpriced'` verbatim + four token tiers), unblocking the bareguard §3.8 consume side. **Remaining:** `memory.facts` (no writer until the remember-consolidation pass — honestly omitted, not zeroed); L10 refusal-billing guard (unbuilt); the bareguard companion item-2 + the real meter→gate round-trip test (separate repo + a devDep bump); the F1 live rubric/agentic calibration *run* (needs a provider key). The whole suite is built.
...
- **Predicate path:** `pass = !!predicate(result)`, `score = null`, `critique = ''` (or a caller-supplied message). Synchronous predicate or one returning a Promise both accepted. No provider needed.
...
| remember (consolidation) | distillation pass | harvest → Memory facts/episodes/prefs | **future glue, out of scope here** |
...
Stash does **not** classify into facts/episodes/prefs — that semantic distillation needs an LLM pass over many spans and belongs to the (future) remember-consolidation into `Memory`. Stash's harvest is the **feedstock** for that later pass: `compact → harvest → (future) consolidate → Memory`.
...
- **Stance side → litectx `episode`.** The recall-able "what this sub-task became" is written as an `episode`: **session-scoped, auto-pruned** (30-day window + `promotionCandidates()` ladder), **upsert-by-key** (latest stance wins), **promotable to `fact`** (= the future remember-consolidation, already shipped in litectx). litectx owns the pruning; bareagent only writes. This doubles as the **structured handoff** the brief prizes ("compaction does not cure coherence; structured handoffs do").
...
- **No semantic classification in stash** (facts/episodes/prefs) — distillation is litectx's `episode→fact` promotion ladder, not stash's job.
...
The §3.6 shape lists more than the meter can honestly source today. **Shipped (2026-06-24):** `metrics.spawned`, `metrics.context.{compactions,summaries,tokensTrimmed}`, and `metrics.memory.{stashed,episodes,recalls}`. The two below were deferred in the original cut (they had no source and would emit silent zeros, the anti-pattern §3.7 rejects); both are now resolved except `facts`:
...
2. **`metrics.memory.{stashed,episodes,recalls}`** — ✅ BUILT via **channel A**: the loop lends a non-enumerable `ctx.recordMemoryOp(kind)` (mirroring `ctx.summarize`); the originating module announces and the loop counts + emits `loop:memory`. Bounded PER RUN (the hook re-attaches each `loop.run()` and closes over that run's meter; `result.metrics` is a copy). `stashed` (lossless parks) + `episodes` (stance writes) flow through the stash fold; `recalls` is opt-in — `Memory.search(query,{ctx})` routes the same hook (no API break, Memory stays Loop-agnostic). **`facts` remains OMITTED, not zeroed** — nothing writes facts until the remember-consolidation pass exists (a 0 would be a false "tracked-and-didn't-happen" signal, §3.7). Metering `recalls` deliberately did NOT thread `ctx` into the Memory wrapper's API (would break "Memory doesn't know Loop"); the opt-in `options.ctx` hook preserves that boundary.
