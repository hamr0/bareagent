...
**Status:** DELIVERED (2026-06-24). Features 1–4 built, validated, and shipped; the §3.8 meter→gate cost contract is closed end-to-end against bareguard 0.9.0. Only out-of-scope line not built: `metrics.memory.facts` (the litectx `episode→fact` consolidation, which this doc scopes out as future glue). Companion spec to the main `prd.md` (which stays THE core one-shot-runner PRD) — this is the design/decisions record for a small, opt-in suite of *assistive* components a consumer composes onto the Loop, never machinery the Loop imposes.
**Owner:** hamr0
...
The §3.6 shape lists more than the meter can honestly source today. **Shipped (2026-06-24):** `metrics.spawned`, `metrics.context.{compactions,summaries,tokensTrimmed}`, and `metrics.memory.{stashed,episodes,recalls,stored}`. The two below were deferred in the original cut (they had no source and would emit silent zeros, the anti-pattern §3.7 rejects); both are now resolved except `facts`:
...
2. **`metrics.memory.{stashed,episodes,recalls,stored}`** — ✅ BUILT via **channel A**: the loop lends a non-enumerable `ctx.recordMemoryOp(kind)` (mirroring `ctx.summarize`); the originating module announces and the loop counts + emits `loop:memory`. Bounded PER RUN (the hook re-attaches each `loop.run()` and closes over that run's meter; `result.metrics` is a copy). `stashed` (lossless parks) + `episodes` (stance writes) flow through the stash fold. The Memory wrapper is metered **symmetrically — read and write** (both opt-in, no API break, Memory stays Loop-agnostic): `recalls` = `Memory.search(query,{ctx})` reads, `stored` = `Memory.store(content,metadata,{ctx})` writes (ctx rides in a trailing opts arg, never in the persisted `metadata`).
   ...
   - **`facts` remains OMITTED, not zeroed** — it is the litectx promotion specifically, which is litectx-internal and has no writer until the remember-consolidation pass exists (a 0 would be a false "tracked-and-didn't-happen" signal, §3.7). Surfacing it is litectx's job via `promotionCandidates`/`recentActivity`, not the meter's.
